...
pub struct CurveConstants<F: Field> {
    ...
    pub c0n: F,
    pub c1n: F, // neg: c_i = 2·y_i + h_i
    pub c0p: F,
    pub c1p: F,
    pub c5p: F, // pos: c0,c1 = y_i + h_i, c5 = 2y1 + h1
    pub c2: F,
    pub c3: F,
    pub c4: F,
...
fn deg2_dbl_neg<F: Field>(u1: F, u0: F, v1: F, v0: F, cc: &CurveConstants<F>) -> DivisorCoords<F> {
    ...
    if sp1.is_zero() {
        ...
        let vpp0 =
            c3 * (upp0 * (s0 * u1 + d5 * (v1 - y1) - upp0 * (s0 - d6 + upp0)) - s0 * u0) - t0;
        super::branch("ADBL11");
        return DivisorCoords::deg1(upp0, yn1, vpp0, 1);
    }
...
fn deg2_dbl_pos<F: Field>(u1: F, u0: F, v1: F, v0: F, cc: &CurveConstants<F>) -> DivisorCoords<F> {
    ...
    if sp1.is_zero() {
        ...
        let upp0 = s0 + d6 - u1 - w4 * d5 * z1;
        let vpp0 =
            c3 * (upp0 * (s0 * u1 + d5 * (c1 + v1) - upp0 * (d6 + s0 - upp0)) - s0 * u0) - t0;
