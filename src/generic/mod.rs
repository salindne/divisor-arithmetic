...
mod comparison_tests {
    use super::*;
    use crate::field::PrimeField;
    use crate::g2::ramified::arbitrary::{CurveConstants, DivisorCoords, add as g2_add, double as g2_double};
    ...
    fn create_valid_deg1_divisor(f: &Poly<F31>, exclude_a: Option<F31>) -> Option<(DivisorCoords<F31>, ramified::Divisor<F31>)> {
        for a_val in 0..31u64 {
            let a = F31::new(a_val);
    ...
    fn test_generic_vs_specialized_add_deg1() {
        ...
        let (d1_coords, d1_generic) = create_valid_deg1_divisor(&f, None)
            .expect("Should find first valid divisor");
        
        // Get the x-coordinate of first divisor to exclude
        let first_a = -d1_coords.u0;
        
        let (d2_coords, d2_generic) = create_valid_deg1_divisor(&f, Some(first_a))
            .expect("Should find second valid divisor");
    ...
    fn test_generic_vs_specialized_double_deg1() {
        ...
        let (d_coords, d_generic) = create_valid_deg1_divisor(&f, None)
            .expect("Should find valid divisor");
        ...
        if d1_deg2_generic.u.deg() == 2 {
            ...
            if let Some((d3_coords, d3_generic, third_a)) = d3_opt {
                ...
                if d2_deg2_generic.u.deg() == 2 {
                    ...
                    assert!(rem.is_zero(), "Generic result should satisfy curve equation");
